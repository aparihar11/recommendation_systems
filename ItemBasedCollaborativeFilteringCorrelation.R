...
UserArtistsMatrix[is.na(UserArtistsMatrix)] <- 0

prediction <- matrix(, nrow=nrow(UserArtistsMatrix), ncol=ncol(UserArtistsMatrix), dimnames=list(rownames(UserArtistsMatrix), colnames(UserArtistsMatrix)))
TopN <- matrix(, nrow=nrow(UserArtistsMatrix), N, dimnames=list(rownames(UserArtistsMatrix)))

for (u in rownames(UserArtistsMatrix)){
  ### Numerator ###
  # Calculate Numerator
  Num <-  UserArtistsMatrix[u, ] %*% similarity_matrix_NN
